...
@interface _FOXDataSetPlaceholderViewButton : UIButton
@end
@implementation _FOXDataSetPlaceholderViewButton
...
@interface FOXDataSetPlaceholderView ()<UIGestureRecognizerDelegate>
...
@interface UIView (FOXDataSetPlaceholderViewOffset)
...
@implementation UIView (FOXDataSetPlaceholderViewOffset)
...
@implementation FOXDataSetPlaceholderView
...
- (UIButton *)defaultButton{
    UIButton *button = [_FOXDataSetPlaceholderViewButton new];
    ...
    [button addTarget:self action:@selector(_placeHolderViewButtonDidTap:) forControlEvents:UIControlEventTouchUpInside];
    
    return button;
}

- (UIButton *)contentViewButton {
    UIButton *button = [_FOXDataSetPlaceholderViewButton new];
    button.backgroundColor = [UIColor clearColor];
    button.enabled = [self _isTouchAllowed];
    [button addTarget:self action:@selector(_contentViewDidClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)_initialize{
    ...
    UIButton *contentButton = [self contentViewButton];
    [self.contentView addSubview:contentButton];
    [contentButton mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self.contentView);
    }];
    
    UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(_tapView:)];
...
- (BOOL)_isTouchAllowed {
    if (self.delegate && [self.delegate respondsToSelector:@selector(placeholderViewShouldAllowTouch)]) {
        return [self.delegate placeholderViewShouldAllowTouch];
    }
    
    return YES;
}

- (void)_contentViewDidClick:(UIButton *)btn {
    if (self.delegate && [self.delegate respondsToSelector:@selector(placeholderView:didTapView:)]) {
        [self.delegate placeholderView:self didTapView:self.contentView];
    }
}


- (void)_tapView:(UIGestureRecognizer *)gesture {
    if (self.delegate && [self.delegate respondsToSelector:@selector(placeholderView:didTapView:)]) {
        [self.delegate placeholderView:self didTapView:self.contentView];
    }
}
...
- (void)_placeHolderViewButtonDidTap:(id)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(placeholderView:buttonDidTap:)]) {
        [self.delegate placeholderView:self buttonDidTap:sender];
    }
}
